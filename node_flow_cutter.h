...
namespace flow_cutter{
	...
	namespace expanded_graph{
		...
		template<class OriginalArcWeight>
		struct ArcWeight{
			int original_node_count, original_arc_count;
			OriginalArcWeight original_arc_weight;
			...
			int operator()(int a)const{
				if(is_expanded_intra_arc(a, original_arc_count)){
					...
					return original_arc_weight(expanded_inter_arc_to_original_arc(a, original_arc_count));
				}
			}
		};

		template<class OriginalArcWeight>
		ArcWeight<OriginalArcWeight>arc_weight(int original_node_count, int original_arc_count, const OriginalArcWeight&original_back_arc){
			return {original_node_count, original_arc_count, std::move(original_back_arc)};
		}
		...
		template<class Tail, class Head, class BackArc, class ArcWeight, class OutArc>
		Graph<
			expanded_graph::Tail<Tail>, 
			expanded_graph::Head<Head>, 
			expanded_graph::BackArc<BackArc>, 
			expanded_graph::ArcWeight<ArcWeight>, 
			expanded_graph::Capacity, 
			expanded_graph::OutArc<OutArc>
		>
		make_graph(Tail tail, Head head, BackArc back_arc, ArcWeight arc_weight, OutArc out_arc){
			...
			return{
				...
				expanded_graph::arc_weight(node_count, arc_count, std::move(arc_weight)),
